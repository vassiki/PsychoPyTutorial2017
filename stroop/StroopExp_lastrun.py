...
expName = u'StroopExp'  # from the Builder filename that created this script
...
thisExp = data.ExperimentHandler(name=expName, version='',
    ...
    originPath=u'/Users/vassiki/Desktop/PsychoPyTutorial2017/stroop/StroopExp.psyexp',
...
text_2 = visual.TextStim(win=win, name='text_2',
    text=u'Remember to respond to the\ncolor, not the word!\n\nleft = red\ndown = green\nright = blue',
    font=u'Arial',
...
text_3 = visual.TextStim(win=win, name='text_3',
    text=u'You can now take a short break!\n\nPress the space bar when you \nare ready to continue!',
    font=u'Arial',
    pos=(0, 0), height=0.1, wrapWidth=None, ori=0, 
    color=u'white', colorSpace='rgb', opacity=1,
    depth=0.0);
...
for thisTrial in trials:
    ...
    # keep track of which components have finished
    trialComponents = [text, trialResp]
    ...
    while continueRoutine:
        ...
        frameRemains = 0.5 + 5.0- win.monitorFramePeriod * 0.75  # most of one frame period left
        if text.status == STARTED and t >= frameRemains:
            text.setAutoDraw(False)
        
        # *trialResp* updates
        if t >= 0.5 and trialResp.status == NOT_STARTED:
            # keep track of start time/frame for later
            trialResp.tStart = t
...
# keep track of which components have finished
restComponents = [text_3, key_resp_3]
...
for thisTrial_2 in trials_2:
    ...
    trials_2.addData('trialResp.keys',trialResp.keys)
    trials_2.addData('trialResp.corr', trialResp.corr)
    if trialResp.keys != None:  # we had a response
        trials_2.addData('trialResp.rt', trialResp.rt)
    # the Routine "trial" was not non-slip safe, so reset the non-slip timer
    routineTimer.reset()
    thisExp.nextEntry()
...
for thisTrial_3 in trials_3:
    ...
    trials_3.addData('trialResp.keys',trialResp.keys)
    trials_3.addData('trialResp.corr', trialResp.corr)
    if trialResp.keys != None:  # we had a response
        trials_3.addData('trialResp.rt', trialResp.rt)
    # the Routine "trial" was not non-slip safe, so reset the non-slip timer
    routineTimer.reset()
    thisExp.nextEntry()
